...
class InteractiveNode(QGraphicsEllipseItem):
    def __init__(self, name, graph_viewer, x, y):
        ...
        self.setBrush(QBrush(QColor("#8e44ad")))  # Purple glow
        self.setAcceptHoverEvents(True)
        self.edges = []

    def hoverEnterEvent(self, event):
        self.setBrush(QBrush(QColor("#c0392b")))  # Highlight node
        for edge in self.edges:
            edge.setPen(QPen(QColor("#c0392b"), 2))

    def hoverLeaveEvent(self, event):
        self.setBrush(QBrush(QColor("#8e44ad")))
        for edge in self.edges:
            edge.setPen(QPen(QColor("#555"), 1))
